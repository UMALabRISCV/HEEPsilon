...
// Input data (must match memory.csv)
// First element is placeholder for result address (for SWI)
int32_t input_data[] __attribute__((aligned(4))) = {
    0, // Will be replaced by &result
    101, 110, -1
};  // Sentinel
...
int main(void) {
    ...
    int32_t step = 0;
    int32_t* ptr = input_data + 1; // Skip the address placeholder
    ...
    plic_Init();
    // Inject result address into input stream for SWI
    ...
    // Column 0 reads from input_data + 1 (skip the address)
    cgra_set_read_ptr(&cgra, (uint32_t)(input_data + 1), 0);
    // Column 1 reads from input_data (to get the address)
    ...
    // Print all CGRA outputs (unconditionally for debugging)
