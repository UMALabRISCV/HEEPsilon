...
\subsection{SELF: Resultado Propio Registrado}

El operando \texttt{SELF} es una de las fuentes mas importantes pero menos 
documentadas del CGRA. Su comprension es fundamental para disenar kernels 
correctos.

\subsubsection{Definicion}

\texttt{SELF} corresponde al \textbf{resultado registrado del ciclo anterior} 
del mismo PE, NO al resultado combinacional del ciclo actual.

\textbf{RTL:} En \texttt{datapath.sv}, linea 219:
\begin{verbatim}
assign mux_a_inputs[ 1] = own_res_i;  // SELF
\end{verbatim}

Y en \texttt{cgra\_rcs.sv}, lineas 354 y 413:
\begin{verbatim}
rcs_mesh_res[k][l] = rcs_res_reg[k][l];  // Valor REGISTRADO
// ...
.own_res_i ( rcs_mesh_res[i][j] ),       // Conectado a SELF
\end{verbatim}

\subsubsection{Diagrama de Temporizado}

\begin{verbatim}
Ciclo:        |  N-1  |   N   |  N+1  |  N+2  |
              +-------+-------+-------+-------+
ALU calcula:  | val_A | val_B | val_C | val_D |
              +-------+-------+-------+-------+
SELF vale:    |   X   | val_A | val_B | val_C |
              +-------+-------+-------+-------+
                       ^^^^^^^
        SELF en ciclo N = resultado ALU de ciclo N-1
\end{verbatim}

\begin{mdframed}[backgroundcolor=lightgray, linecolor=warnorange, linewidth=1pt]
\textbf{Punto Critico:} Es imposible usar el resultado de la ALU en el 
\textbf{mismo ciclo} como operando. Si calculas un valor en ciclo $t$, 
solo puede usarse como \texttt{SELF} (o \texttt{RCx}) en ciclo $t+1$.
\end{mdframed}

\subsubsection{Comparacion: SELF vs Registros (R0-R3)}

\begin{table}[h]
\centering
\caption{Diferencias entre SELF y Registros Internos}
\begin{tabular}{p{3cm}ccp{4cm}}
\toprule
\textbf{Caracteristica} & \textbf{SELF} & \textbf{R0-R3} & \textbf{Notas} \\
\midrule
Fuente & ROUT registrado & RF interno & Diferentes registros \\
Latencia escritura & 1 ciclo & 1 ciclo & Ambos tardan 1 ciclo \\
Visible a vecinos & Si (\texttt{RCx}) & No & Solo ROUT se propaga \\
Escritura explicita & No (automatico) & Si (\texttt{RF\_WE=1}) & ROUT siempre se actualiza \\
Persistencia & 1 ciclo & Hasta overwrite & R0-R3 retienen valor \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Casos de Uso Comunes}

\textbf{1. Acumulador (patron incorrecto vs correcto):}

\begin{verbatim}
; INCORRECTO - SELF no tiene el valor que esperas!
LWD  -,   -,  4       ; Ciclo 0: Carga dato -> ROUT = dato
SADD SELF, RCB, -     ; Ciclo 1: SELF = dato(ok), resultado -> ROUT
SADD SELF, RCB, -     ; Ciclo 2: SELF = suma anterior (ok!)

; CORRECTO - El primer SELF en ciclo 1 es el LWD del ciclo 0
; Este patron funciona para acumulacion continua
\end{verbatim}

\textbf{2. Store a memoria usando SELF:}

\begin{verbatim}
; El valor a guardar esta en SELF (calculado en ciclo anterior)
SWD  SELF, -, -, 4    ; MUX_A = SELF = valor a escribir
                      ; El campo IMM (4) es el incremento del puntero
\end{verbatim}

\textbf{3. Patron de suma con latencia (correcto):}

\begin{verbatim}
Ciclo 0: SADD R0, R1, -     ; Calcula R0+R1 -> ROUT
Ciclo 1: SADD SELF, R2, -   ; SELF = (R0+R1), calcula + R2 -> ROUT
Ciclo 2: SWD  SELF, -, 4    ; SELF = ((R0+R1)+R2), escribe a memoria
\end{verbatim}

\subsubsection{SELF vs RCx (Vecinos)}

Todos siguen la misma regla de latencia de 1 ciclo:

\begin{verbatim}
PE(row, col):
  SELF = valor de ROUT[row][col] del ciclo anterior
  RCL  = valor de ROUT[row][col-1] del ciclo anterior (izquierda)
  RCR  = valor de ROUT[row][col+1] del ciclo anterior (derecha)
  RCT  = valor de ROUT[row-1][col] del ciclo anterior (arriba)
  RCB  = valor de ROUT[row+1][col] del ciclo anterior (abajo)
\end{verbatim}

\textbf{RTL:} \texttt{cgra\_rcs.sv}, lineas 349-378, donde \texttt{rcs\_mesh\_res} 
conecta los registros de salida en topologia toroidal.

\subsubsection{NOP y Preservacion de SELF}

\textbf{RTL:} \texttt{cgra\_rcs.sv}, lineas 242-245:
\begin{verbatim}
if (rcs_nop_s[k][j] == 1'b0) begin        // No es NOP
  rcs_res_reg[k][j]  <= rcs_res[k][j];    // Actualiza ROUT
end
// Si es NOP, rcs_res_reg MANTIENE su valor anterior
\end{verbatim}

\textbf{Implicacion:} Durante un \texttt{NOP}, el valor de \texttt{SELF} 
(y lo que los vecinos ven via \texttt{RCx}) permanece \textbf{sin cambiar}.

\subsubsection{SELF despues de LWD}

Cuando se ejecuta \texttt{LWD}, el dato leido de memoria va directamente 
al registro de salida (\texttt{rcs\_res\_reg}):

\textbf{RTL:} \texttt{cgra\_rcs.sv}, lineas 246-256:
\begin{verbatim}
if (data_req_s[k][j] == 1'b1) begin       // Hay request de datos
  if (rvalid_demux[j][k] == 1'b1) begin   // Dato disponible
    rcs_res_reg[k][j] <= data_rdata_i[j]; // Dato -> ROUT
  end
end
\end{verbatim}

Por lo tanto, despues de un \texttt{LWD} (o \texttt{LWI}), \texttt{SELF} 
contendra el dato leido de memoria, disponible en el ciclo siguiente.
